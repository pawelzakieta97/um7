...
class UM7array(object):
    ...
    def __init__(self, names, ports, statevars, baud=115200):
        ...
        self.history = np.zeros(len(statevars))
        for i in statevars:
    ...
    def updatehistory(self):
        ...
        for i in self.statevars:
            state = np.append(state, self.state[i])
        self.history = np.vstack((self.history, state))
...
class UM7(object):
    ...
    def __init__(self, name, port, statevars, baud=115200):
        ...
        self.statemask = {}
        self.statevars = statevars
        self.serial = serial.Serial(port, baudrate=baud, bytesize=8, parity='N', stopbits=1,
                                    timeout=0)  # Open serial device
        self.registers = np.zeros(180)
    ...
    def readpacket(self):
        ...
        if foundpacket == 0:
            ...
            try:
                read_data = self.serial.read(size=1)
                #print(read_data[0])
                ptbyte = bin(int(binascii.hexlify(read_data), 16))[2:]
                has_data = (read_data[0] & 0x80) == 0x80
                is_batch = (read_data[0] & 0x40) == 0x40
                batch_length = (read_data[0] >> 2) & 0x0F
                batch_length *= 4
                print(has_data, is_batch, batch_length)

                commandfailed = int(ptbyte[7], 2)
                read_data = self.serial.read(size=1)
                startaddress = int(read_data[0])
                print(startaddress)
                if has_data:
                    data = self.serial.read(size=batch_length)
                    data = np.array([int(element) for element in data])
                    print(data)
                else:
                    data = False
            except ValueError:
                ...
                startaddress = 0
                data = 0
        return [foundpacket, has_data, startaddress, data, commandfailed]
...
def parsedatabatch(data, startaddress, devicename):
    ...
    try:
        if startaddress == 97:  # Processed Gyro Data
            ...
            print('euler')
            roll = int2int16((data[0]<<8)+data[1])/ 91.02222
            pitch = int2int16((data[2]<<8)+data[3])/ 91.02222
            yaw = int2int16((data[4]<<8)+data[5])/ 91.02222
            #print(data[0], pitch, yaw)
            # n = 4
            # datasplit = [data[i:i + n] for i
            #              in range(0, len(data), n)]  # Split data string into array of data bytes (n hex chars each)
            # del datasplit[9]
